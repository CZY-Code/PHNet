...
class ToTensor(object):
    ...
    """
    def __init__(self, keys=['img', 'lane_line', 'seg', 'flow', 'occlusion'], cfg=None):
        self.keys = keys
...
class GenerateLaneLine(object):
    ...
    def sample_lane(self, points, sample_ys):
        ...
        return extrap_xs, interp_xs, xs_outside_image, xs_inside_image
    ...
    def transform_annotation(self, anno):
        ...
        # 2 scores, 1 start_y, 1 start_x, 1 theta, 2 length, S coordinates 79个数据
        lanes = np.ones((self.max_lanes, 2 + 2 + 1 + 2 + self.n_offsets), dtype=np.float32) * -1e5  
        ...
        for lane_idx, lane in zip(anno['lanes_ids'], old_lanes):
            if lane_idx >= self.max_lanes:
                break
            try:
                xs_extrap, xs_interp, xs_outside_image, xs_inside_image = self.sample_lane(lane, self.offsets_ys)
            ...
            lanes[lane_idx, 5] = len(xs_interp) / self.n_strips
            lanes[lane_idx, 6] = (len(xs_extrap)-len(xs_outside_image)) / self.n_strips
            lanes[lane_idx, 7:7 + len(all_xs)] = all_xs #实际位置 0～640
    ...
    def __call__(self, sample):
        ...
        sample['lane_line'] = label #72+7个点用于回归
