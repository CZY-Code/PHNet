...
DATA_CONTAINER = {}
ROOT = opt.root
MAX_LANES = opt.max_lanes #车道线分割实例数目
MAX_TRAINING_SKIP = 100


def multibatch_collate_fn(batch):
    # batch的长度为1，sample的长度为16 sample[0] = [data, num_obj, info]
    ...
    occlusions = torch.stack([torch.stack([data['occlusion'] for data in sample[0]]) for sample in batch]) #[1, 16, 8]

    try:
        infos = [sample[2] for sample in batch]
    except IndexError as ie:
        infos = None
    return frames, masks, lanes_lines, objs, infos, flows, occlusions
...
class VIL(BaseData):
    def __init__(self, train=True, sampled_frames=3,
        ...
        self.sampled_frames = sampled_frames #16
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            nsamples = min(self.sampled_frames, nframes) #min(16, 100)
        ...
        for name in sample_frame: #8
            ...
            with open(os.path.join(jsonfolder, name+ '.jpg' + '.json')) as f:
                ...
                lanes_anno = []
                lanes_ids = []
                lanes_occ = [-1]*self.max_obj
                for lane in lanes_info['annotations']['lane']:
                    lane_id = lane['lane_id']-1
                    lanes_ids.append(lane_id)
                    lanes_anno.append(lane['points'])
                    lanes_occ[lane_id] = lane['occlusion']
                sample['lanes_ids'] = lanes_ids
                sample['lanes'] = lanes_anno #按照cut_height_lanes函数的判断，车道以左上角为原点
                sample['occlusion'] = np.array(lanes_occ)
