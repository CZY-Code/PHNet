...
from libs.models.Router4OLV2 import RouterOL
...
opt = Config.fromfile('./options4OLV3.py')
ROOT = opt.root
            ...
            for idx in range(N):
                ...
                for i, imgList in enumerate(imgLists): # N=1 逐clip进行分析
                    ...
                    for t, lanes in enumerate(lane_lines):
                        ...
                        # predlanesV2(info, lanes, T//2*i+t) #检测结果可视化
                end = time.time()                
                tmp_time = end - start
...
def predlanesV2(info, lanes, img_num, show=True, width=4):
    size = info['size']
    vidname = info['name']
    ...
    for idx, xys in enumerate(lanes_xys):
        for i in range(1, len(xys)):
            ...
            # cv2.line(seg_show, xys[i - 1], xys[i], COLORS[0], thickness=30) #30
    ...
    if True:        
        ...
        video = os.path.join('./output/visualization/openlane/', vidname)
        if not os.path.exists(video):
            os.makedirs(video)
        cv2.imwrite(os.path.join(video, output_name), img)
