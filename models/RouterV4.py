...
class RouterV2(nn.Module):
    def __init__(self,
        ...
        # n offsets + start_y + start_x + theta + valid_len + invalid_len
        self.reg_layers = nn.Linear(self.reg_hidden_dim, self.n_offsets + 2 + 1 + 2) #是否要加一个激活函数？？？
        ...
        self.reg_layers_sec = nn.Linear(self.reg_hidden_dim*2, self.n_offsets + 2 + 1 + 2) #是否要加一个激活函数？？？
        self.cls_layers_sec = nn.Linear(self.reg_hidden_dim*2, 2)
        # self.reg_layers_sec = nn.Linear(self.reg_hidden_dim, self.n_offsets + 2 + 1 + 2) #是否要加一个激活函数？？？
    ...
    def generate_priors_from_embeddings(self):
        ...
        # 2 scores, 1 start_y, 1 start_x, 1 theta, 2 length, 72 coordinates
        priors = predictions.new_zeros(
            (self.num_priors, 2 + 2 + 1 + 2 + self.n_offsets), device=predictions.device)
        ...
        priors_on_featmap = priors.clone()[..., 7 + self.sample_x_indexs] #在72个点中均匀取36个点 不共享内存
    ...
    def forward(self, x, last_cuts=None):
        ...
        for stage in range(self.refine_layers): # 进行三次pooling
            ...
            if stage != self.refine_layers - 1: 
                ...
                priors_on_featmap = priors[..., 7 + self.sample_x_indexs]
    ...
    def forward_first(self, decode_feat_l, priors):
        ...
        reg = reg.reshape(batch_size, self.num_priors, self.n_offsets + 2 + 1 + 2)
        ...
        predictions[:, :, 5] = reg[:, :, 3] # valid len
        predictions[:, :, 6] = reg[:, :, 4] # invalid len
    ...
    def forward_second(self, last_cut, attn_feat, priors):
        ...
        prediction_lines = predictions.clone()
        #预测的offsets 每次全连接预测的offset都应该是相对于原始anchor的尺度
        predictions[..., 7:] += reg[..., 5:] # 预测的offsets
        return predictions, prediction_lines
    ...
    def predictions_to_pred(self, predictions, ori_img_h, cut_height=0):
        ...
        for lane in predictions:
            lane_xs = lane[7:]  # normalized value
            invalid_len = int(round(lane[6].item()))
            start = min(max(0, int(round(lane[2].item() * self.n_strips))),
                        self.n_strips) + invalid_len
            
            valid_len = int(round(lane[5].item()))
            end = start + valid_len - 1
    ...
    def get_lanes(self, output, org_size, cut_scale=0, as_lanes=True):
        ...
        for predictions in output: #循环epoch
            ...
            #删除invalid length
            nms_predictions = torch.cat([nms_predictions[..., :6], nms_predictions[..., 7:]], dim=-1)
            #删除theta
            ...
            predictions[:, 6] = torch.round(predictions[:, 6] * self.n_strips)
    ...
    def get_labels(self, labels, as_lanes=True):
        ...
        for predictions in labels: #循环batch
            ...
            predictions[:, 7:] /= (self.img_w - 1) #points
...
class RouterWithB(nn.Module):
    ...
    def forward(self, inputs:dict()):
        ...
        for t in range(T): #0-1 循环clip进行逐帧分析
            curr_feat = tuple(curr_feats[i][t:t+1] for i in range(len(curr_feats)))
            outputs, curr_cut, diff_list = self.detNet(curr_feat, last_cuts)
            if self.training:
                gt_lane = lanes[t:t+1]
                ...
                lane_lines = torch.where(diffOneFrame>=0.5, outputs['predictions_sec'][-1], outputs['predictions_fir'][-1])
                lane_lines, keep_inds, keep = self.detNet.get_lanes(lane_lines, info['size'], self.cut_scale) #第一张图的车道线list
