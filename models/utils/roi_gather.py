...
class ROIGather(nn.Module):
    ...
    def roi_fea(self, roi_features, layer_index):
        feats = []
        for i, feature in enumerate(roi_features): #循环stage
    ...
    def forward(self, roi_features, x, layer_index):
        '''
        Args:
            roi_features: prior feature, list of tensor with shape: (Batch * num_priors, prior_feat_channel, sample_point, 1)
            x: feature map
        ...
        '''
        roi = self.roi_fea(roi_features, layer_index) #[24*192, 64, 36, 1]
        bs = x.size(0)
        roi = roi.contiguous().view(bs * self.num_priors, -1) #[24*192, 64*36]

        roi = F.relu(self.fc_norm(self.fc(roi))) #[24*192, 64]
        roi = roi.view(bs, self.num_priors, -1) #[24, 192, 64]
        
        query = roi
        value = self.resize(self.f_value(x)) #特征图 BCHW
        query = self.f_query(query) #[24, 192, 64]  #切出来的特征向量
        key = self.f_key(x)
        value = value.permute(0, 2, 1) #[24, 250, 64]
        key = self.resize(key) #[24, 64, 250]

        sim_map = torch.matmul(query, key) # 192条线和全图每个点的的attention
...
class ROIGather_T(nn.Module):
    ...
    def __init__(self,
                 ...
                 refine_layers,
                 mid_channels=48):
        super(ROIGather_T, self).__init__()
        self.in_channels = in_channels
        self.num_priors = num_priors
        self.f_key = ConvModule(in_channels=self.in_channels,
                                out_channels=self.in_channels,
