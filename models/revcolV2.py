...
from libs.models.RevCol.modules import ConvNextBlock, Decoder, LayerNorm, SimDecoder, UpSampleConvnext
from libs.models.RevCol.revcol_function import ReverseFunction
...
class FullNet(nn.Module):
    def __init__(self, channels=[64, 128, 256, 512], layers=[2, 2, 4, 2], num_subnet=4, kernel_size = 3, drop_path = 0.0, save_memory=True, inter_supv=True, head_init_scale=None) -> None:
        super().__init__()
        ...
        if not inter_supv:
            ...
            if True:
                ...
                self.decoder_blocks = nn.ModuleList([SimDecoder(in_channel=channels[-1], encoder_stride=8) for _ in range(3) ])
    ...
    def forward(self, x):
        if self.inter_supv:
            ...
            for i in range(self.num_subnet):
                ...
                x_img_out.append(c0) #[取所有子网络的最高level]
            return x_img_out
    
    def _forward_intermediate_supervision(self, x):
        x_img_out = []
        c0, c1, c2, c3 = 0, 0, 0, 0
        interval = self.num_subnet//4 #interval = 1 
        
        x = self.stem(x)        
        for i in range(self.num_subnet):
            c0, c1, c2, c3 = getattr(self, f'subnet{str(i)}')(x, c0, c1, c2, c3)
            if (i+1) % interval == 0: #i = [0,1,2,3]
                if i != self.num_subnet-1: #i=[0,1,2] 对较低level进行重建？
                    x_img_out.append(self.decoder_blocks[i//interval](c3)) #希望返回decoder使用的特征
        return x_img_out
...
def revcol_tiny(save_memory, inter_supv=True, drop_path=0.1, kernel_size = 3):
