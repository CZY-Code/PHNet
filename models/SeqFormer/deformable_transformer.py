...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            bs, nf, c, h, w = src.shape #[N, nf, c, h, w]
            spatial_shape = (h, w)
        ...
        src_flatten = torch.cat(src_flatten, 2) #[1, 5, 4250, 256]
        mask_flatten = torch.cat(mask_flatten, 2) #[1, 5, 4250]
        ...
        bs, nf,  _, c = memory.shape #[1, 5, 4250, 256]
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        src = src + self.dropout1(src2) #[5, 1, 4250, 256]
        src = self.norm1(src)
        # ffn
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        output = src
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device) #[5, 4250, 4, 2]
