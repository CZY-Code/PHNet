    ...
    def __init__(self, backbone, transformer, num_classes, num_frames, num_queries, num_feature_levels,
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                in_channels = backbone.num_channels[_] #512
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                    nn.GroupNorm(32, hidden_dim), #按通道分为32组的LayerNorm
                ))
            for _ in range(num_feature_levels - num_backbone_outs):
                in_channels = backbone.num_channels[-1] #by czy
                input_proj_list.append(nn.Sequential(
        ...
        if with_box_refine:
            self.class_embed = _get_clones(self.class_embed, num_pred) #len(self.class_embed)=6
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, losses, focal_alpha=0.25, mask_out_stride=4, num_frames=5):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes[idx] = target_classes_o.min(1)[0]
        ...
        if log:
            ...
            losses['class_error'] = 100 - accuracy(src_logits[idx], target_classes_o.min(1)[0])[0]
        return losses
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_idx = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)])
        target_src_x, target_idx_y = torch.where(target_idx<8)
        target_idx_x = target_idx[torch.where(target_idx<8)]
        target_vaild_idx = (target_idx_x, target_idx_y)

        
        tgt_bbox = torch.cat([v["boxes"] for v in targets])[(target_idx_x, target_idx_y)]
        src_boxes = src_boxes[(target_src_x, target_idx_y)]
        ...
        loss_bbox = F.l1_loss(src_boxes, tgt_bbox, reduction='none')
        # loss_giou = 0 #改为点的IoU计算
        ...
        if(losses['loss_bbox']>=1000):
            print(torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)]))
            print(losses['loss_bbox'])
            print(torch.cat([v["boxes"] for v in targets]))
            print(target_vaild_idx)
            print(src_boxes)
            print(tgt_bbox)
            exit(0)
        losses['loss_giou'] = 0 #loss_giou.sum() / num_boxes
    ...
    def forward(self, outputs, targets, indices_list, valid_ratios):
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets, indices_list[-1], num_boxes, **kwargs)) #只用了最后一层的匹配结果

        # In case of auxiliary losses, we repeat this process with the output of each intermediate layer.
        if 'aux_outputs' in outputs: #['pred_logits', 'pred_boxes', 'pred_masks', 'aux_outputs']
