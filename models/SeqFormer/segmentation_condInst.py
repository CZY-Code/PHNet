...
class CondInst_segm(nn.Module):
    def __init__(self, detr, rel_coord=True, freeze_detr=False):
        ...
        self.controller = MLP(hidden_dim, hidden_dim, self.num_gen_params, 3) #parameter maker
    ...
    def forward(self, samples: NestedTensor, gt_targets, criterion, train=False):    
        ...
        poses = []
        spatial_shapes = []
        for l, feat in enumerate(features[1:]): #C3-C5
            # src: [nf*N, _C, Hi, Wi],
            # mask: [nf*N, Hi, Wi],
            # pos: [nf*N, C, H_p, W_p]
        ...
        # features:
        # torch.Size([5, 256, 80, 160]) x4
        # torch.Size([5, 512, 40, 80]) x8
        # torch.Size([5, 1024, 20, 40]) x16
        # torch.Size([5, 2048, 10, 20]) x32

        if self.detr.num_feature_levels > (len(features) - 1): #4>(4-1)
            _len_srcs = len(features) - 1 #3 = 4-1
            for l in range(_len_srcs, self.detr.num_feature_levels): #3
                if l == _len_srcs:
                    src = self.detr.input_proj[l](features[-1].tensors) #upsample C5
                else:
                    src = self.detr.input_proj[l](srcs[-1])
                m = samples.mask    # [nf*N, H, W]
                mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]
                pos_l = self.detr.backbone[1](NestedTensor(src, mask)).to(src.dtype) #position_embedding
        ...
        # print(hs.shape) #[6, 1, 300, 256]
        # print(hs_box.shape) #[6, 1, 5, 300, 256]
        # print(memory.shape) #[1, 5, 4250, 256]
        # print(init_reference.shape) #[1, 5, 300, 2]
        # print(inter_references.shape) #[6, 1, 5, 300, 4]
        # print(valid_ratios.shape) #[1, 2]
        ...
        enc_lay_num = hs.shape[0] #6 
        for lvl in range(enc_lay_num): #6 在每个level上都进行一次预测
            if lvl == 0:
            ...
            dynamic_mask_head_params = self.controller(hs[lvl]) #[bs, num_quries, num_params] condInst生成参数
            ...
            for i, indice in enumerate(indices): #循环bs
                pred_i, tgt_j = indice #pred_i: 和label匹配后的结果 范围：0-300
        ...
        outputs_class = torch.stack(outputs_classes) #[6, 1, 300, 9]
        outputs_coord = torch.stack(outputs_coords) #[6, 1, 5, 300, 4]
        outputs_mask = outputs_masks #6*1*[1, 9, 5, 80, 160]

        outputs['pred_logits'] = outputs_class[-1] #[1, 300, 9]
        outputs['pred_boxes'] = outputs_coord[-1] #[1, 5, 300, 4]
        outputs['pred_masks'] = outputs_mask[-1] #1*[1, num_insts, 5, 80, 160]

        if self.detr.aux_loss: #中间层的损失
        ...
        if not train:
            ...
            # mask prediction outputs['pred_masks']: [bs, num_queries, num_frames, H/4, W/4]
    ...
    def forward_mask_head_train(self, outputs, feats, spatial_shapes, reference_points, mask_head_params, num_insts):
        ...
        outputs['pred_masks'] = output_pred_masks #1*[1, 9, 5, 80, 160]
        return outputs
    ...
    def dynamic_mask_with_coords(self, mask_feats, reference_points, mask_head_params, num_insts, 
                                 mask_feat_stride, rel_coord=True): #同样是逐帧分析
        ...
        if rel_coord:
            ...
            mask_head_inputs = []
            inst_st = 0
            for i, num_inst in enumerate(num_insts):#使用feature的第num_inst维度的特征？
...
def aligned_bilinear(tensor, factor):
    ...
    h, w = tensor.size()[2:]
    tensor = F.pad(tensor, pad=(0, 1, 0, 1), mode="replicate") #pad的参数两个一组从最后一个维度开始padding
    oh = factor * h + 1
    ow = factor * w + 1
...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2):
    ...
    p_t = prob * targets + (1 - prob) * (1 - targets) #[1, 300, 8]
    loss = ce_loss * ((1 - p_t) ** gamma) #[1, 300, 8]
    if alpha >= 0:
