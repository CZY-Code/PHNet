...
from libs.models.utils.dynamic_head import DynamicConvV2
from libs.models.Router import AdaptiveRouter4LaneV2, AdaptiveRouter4LaneV3
from libs.models.revcolV2 import revcol_tiny
from libs.models.resnet import ResNetWrapper
from libs.models.fpnV2 import FPN
...
class Encoder(nn.Module):
    ...
    def forward(self, batch):
        fea = self.backbone(batch['img'] if isinstance(batch, dict) else batch)[:-1]
        fea = self.neck(fea)
        return fea
'''
不同层的通道维度不一样
...
class RouterV2(nn.Module):
    def __init__(self,
                 prior_feat_channels=[64, 32, 16], #[32, 16, 8]
                 reg_hidden_dim=256, #64
                 num_fc=2,
                 refine_layers=3,
                 sample_points=[24, 48, 96],
                 cfg=None):
        ...
        for stage, sample_point in enumerate(self.sample_points):
            self.register_buffer(name=f'sample_x_indexs_{stage}', tensor=(torch.linspace(
                0, 1, steps=sample_point, dtype=torch.float32) * self.n_strips).long())
            self.register_buffer(name=f'prior_feat_ys_{stage}', tensor=torch.flip(
                (1 - getattr(self, f'sample_x_indexs_{stage}').float() / self.n_strips), dims=[-1]))
        ...
        # self.crossFrameAtt = nn.ModuleList([copy.deepcopy(decoder_layer) for _ in range(self.refine_layers)])
        ...
        self.DHead_series = nn.ModuleList()
        for stage in range(self.refine_layers): 
            DHead = DynamicConvV2(feat_size=self.sample_points[stage], 
                                  inplanes=self.prior_feat_channels[stage],
                                  outplanes = self.reg_hidden_dim,
                                  early_return=False)
            self.DHead_series.append(DHead)


        self.pro_embedding = nn.Embedding(self.num_priors, self.reg_hidden_dim)

        self.router = AdaptiveRouter4LaneV2(num_priors=self.num_priors, 
        ...
        # self.router = AdaptiveRouter4LaneV3(num_priors=self.num_priors,
        #                                     inplane=self.reg_hidden_dim,
        #                                     out_channels=1, reduction=4, 
        #                                     stages=self.refine_layers)
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs, stage):
        '''
        ...
        prior_ys = getattr(self, f'prior_feat_ys_{stage}').repeat(batch_size * num_priors).view(
        ...
        feature = feature.reshape(batch_size * num_priors, self.prior_feat_channels[stage], self.sample_points[stage], 1)
        return feature
    ...
    def generate_priors_from_embeddings(self):
        ...
        priors_on_featmap = priors.clone()[..., 6 + getattr(self, f'sample_x_indexs_{0}')] #在72个点中均匀取36个点 不共享内存
    ...
    def forward(self, x, last_cuts=None):
        ...
        priors = self.priors.repeat(batch_size, 1, 1) #[B, 192, 78] 
        priors_on_featmap = self.priors_on_featmap.repeat(batch_size, 1, 1) #[B, 240, 24+48+96]
        # priors_on_featmaps = [p.repeat(batch_size, 1, 1) for p in self.priors_on_featmaps]
        ...
        for stage in range(self.refine_layers): # 进行三次pooling
            # prior_xs = torch.flip(priors_on_featmaps[stage], dims=[2])
            prior_xs = torch.flip(priors_on_featmap, dims=[2])
            #[24*192, 64, 36, 1]
            batch_prior_features = self.pool_prior_features(batch_features[stage], self.num_priors, prior_xs, stage)
            batch_prior_features = batch_prior_features.reshape(batch_size, self.num_priors, 
                                                                self.prior_feat_channels[stage], 
                                                                self.sample_points[stage]) #[B,N,C,H]
            ...
            ##feature enhancement
            roi_feat = batch_prior_features.transpose(2, 3) #[B, 192, 24, 32] [B,A,P,C]
            ...
            attnFeat = contentFeat + posEembed
            ...
            # head_weightMedian = torch.median(head_weight)
            prediction_lines = (1-head_weight) * prediction_fir + head_weight * prediction_sec
            # prediction_lines = torch.where(head_weight>=0.5, prediction_sec, prediction_fir)
            if stage != self.refine_layers - 1: 
                priors = prediction_lines.detach().clone() # 不共享内存也不叠加梯度
                priors_on_featmap = priors[..., 6 + getattr(self, f'sample_x_indexs_{stage+1}')]
    ...
    def forward_second(self, last_cut, attn_feat, stage, priors):
        ...
        # if last_cut is not None and last_cut.shape[0] != 0:
        #     decode_feat_g = self.crossFrameAtt[stage](tgt=attn_feat, memory=last_cut).transpose(0, 1) #[192, 24, 64]
        # else:
        #     decode_feat_g = self.crossFrameAtt[stage](tgt=attn_feat, memory=attn_feat).transpose(0, 1) #[192, 24, 64]
    ...
    def forward(self, inputs:dict()):
        ...
        for t in range(self.save_freq): #0-1 循环clip进行逐帧分析
            ...
            if self.training:
                ...
                lane_lines = torch.where(diffOneFrame>=0.5, outputs['predictions_sec'][-1], outputs['predictions_lists'][-1]) #[1,240,78]
                lane_lines, keep_inds, keep = self.router.get_lanes(lane_lines, self.org_size, self.crop_size) #第一张图的车道线list
        ...
        for t in range(self.save_freq, T): #1-16 循环clip进行逐帧分析
            ...
            if self.training:
                ...
                lane_lines, keep_inds, keep = self.router.get_lanes(lane_lines, self.org_size, self.crop_size) #每一张图的车道线list
