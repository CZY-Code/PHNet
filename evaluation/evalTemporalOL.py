...
def culane_metric2(pred, anno, width=30, iou_threshold=0.5, official=True, img_shape=None):
    if len(pred) == 0:
        return [], [], []
    if len(anno) == 0:
        return [], [], []
...
class LaneEval_Temporal(object):
    ...
    def eval_predictions(self):
        ...
        img_shape = (self.cfg.eval_h, self.cfg.eval_w, 3)

        if self.sequential:
            results_s = t_map(partial(culane_metric2, width=self.lane_width, iou_threshold=self.iou, official=self.official, img_shape=img_shape), predictions, annotations)
        else:
            results_s = p_map(partial(culane_metric2, width=self.lane_width, iou_threshold=self.iou, official=self.official, img_shape=img_shape), predictions, annotations)
        save_pickle(path=f'{self.pred_dir}/results', data=results_s)

        # video metric
        results_s = load_pickle(f'{self.pred_dir}/results') #23021长度
